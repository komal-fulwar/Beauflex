<?php
	...
	$enable_lightbox = iva_get_option( 'enable-image-lightbox' );
	...
	if( !empty( $post_style ) && $post_style != 'custom' ): ?>
        ...
        <article id="post-<?php the_ID();?>" <?php post_class($post_classes);?>><?php
            $template = apply_filters( 'iva_blog_single_template', 'framework/templates/single/post-'.$post_style.'.php' );
		...
		$post_classes = array('blog-single-entry', 'post-custom-style' ); ?>
        ...
        <article id="post-<?php the_ID();?>" <?php post_class($post_classes);?>><?php
			...
			edit_post_link( esc_html__( ' Edit ','iva' ) ); ?>
        </article><?php
	endif;