...
<?php
	...
	if( $post_Style == 'breadcrumb-parallax' ): ?>
		...
		<a href="<?php echo get_author_posts_url(get_the_author_meta('ID', $authid));?>" title="<?php esc_attr_e('View all posts by ', 'iva'); echo get_the_author_meta('display_name', $authid);?>">
			<?php echo get_the_author_meta('display_name', $authid);?>
        </a><?php
	elseif( $post_Style == 'overlay' ): ?>
		<span>
			<?php esc_html_e('Posted by: ', 'iva'); ?>
            <a href="<?php echo get_author_posts_url(get_the_author_meta('ID', $authid));?>" title="<?php esc_attr_e('View all posts by ', 'iva'); echo get_the_author_meta('display_name', $authid);?>">
                <?php echo get_the_author_meta('display_name', $authid);?>
            </a>
        </span><?php
