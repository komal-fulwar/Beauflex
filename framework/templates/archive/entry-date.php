...
<?php
	if( $post_Style == 'dt-sc-simple-style' || $post_Style == 'dt-sc-title-overlay-style' || $post_Style == 'dt-sc-simple-withbg-style' || $post_Style == 'dt-sc-overflow-style'  ):
		echo sprintf( esc_html__( '%s ago', 'iva' ), human_time_diff( get_the_date('U'), current_time('timestamp') ) );
	elseif( $post_Style == 'dt-sc-overlap-style' ):
    	echo esc_html__('on ', 'iva').get_the_date ( get_option('date_format') ).esc_html__(' at ', 'iva').get_the_time('', $post_ID);
