...
class iva_post_functions {

	public $element_position = array();

	function __construct() {

		$elp = iva_get_option( 'blog-elements-position' );
		foreach( $elp as $ep ) {
			$this->element_position[$ep] = $ep;
		}

		add_filter( 'post_class', array( $this, 'iva_add_remove_post_class' ), 10, 1 );

		add_action( 'wp_ajax_iva_ajax_load_more_post', array( $this, 'iva_ajax_load_more_post' ) );
		add_action( 'wp_ajax_nopriv_iva_ajax_load_more_post', array( $this, 'iva_ajax_load_more_post' ) );
	}

	function iva_post_meta_fields( $ajax = false, $page_ID = 0 ) {
		...
		elseif( is_home() || is_search() || is_archive() || $ajax ) :
			...
			$element_position = iva_get_option( 'blog-elements-position' );
			$ele_pos = $meta_ele_pos = array();
			foreach( $element_position as $ep ) {
				$ele_pos[$ep] = $ep;
			}

			$meta_group_position = iva_get_option( 'blog-meta-position' );
			foreach( $meta_group_position as $mp ) {
				$meta_ele_pos[$mp] = $mp;
			}

			$enable_excerpt = iva_get_option( 'enable-excerpt-text' );
			$excerpt_length = iva_get_option( 'blog-excerpt-length' );

			$read_more = iva_get_option( 'blog-readmore-text' );

			$enable_slider = iva_get_option( 'enable-gallery-slider' );

			$enable_video_audio = iva_get_option( 'enable-video-audio' );

			$enable_post_format = iva_get_option( 'enable-post-format' );
	...
	function iva_add_remove_post_class( $classes ) {
	...
	function iva_ajax_load_more_post() {
		...
		if(isset($blogpostloadmore_nonce) && wp_verify_nonce($blogpostloadmore_nonce, 'blogpostloadmore_nonce')) {
			...
			$post_layout = ( isset( $_REQUEST['layout'] ) ) ? $_REQUEST['layout'] : iva_get_option( 'blog-post-layout' );
			$combine_class[] = $post_layout.'-layout';

			$combine_class[] = $post_style = ( isset($_REQUEST['style']) ) ? $_REQUEST['style'] : iva_get_option( 'blog-post-grid-list-style' );
			...
			if($the_query->have_posts()):
				...
				if( empty($meta) ) {
					$obj = new iva_post_functions;
					$meta = $obj->iva_post_meta_fields(true);
				}
				...
				while( $the_query->have_posts() ):
					...
					<div class="<?php echo esc_attr($temp_class);?>">
						<article id="post-<?php the_ID();?>" <?php post_class( array( 'blog-entry', 'format-'.$format ) );?>><?php
							...
							$template = apply_filters( 'iva_blog_archive_template', 'framework/templates/archive-blog-entry.php' );
				...
				<h2><?php esc_html_e('Nothing Found.', 'iva'); ?></h2>
				<p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'iva'); ?></p><?php
			endif;
