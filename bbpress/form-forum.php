			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( esc_html__( 'Now Editing %s', 'iva' ), '&ldquo;'.bbp_get_forum_title().'&rdquo;' );
						else
							bbp_is_single_forum() ? printf( esc_html__( 'Create New Forum in %s', 'iva' ), '&ldquo;'.bbp_get_forum_title().'&rdquo;' ) : esc_html_e('Create New Forum', 'iva' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php esc_html_e('This forum is closed to new content, however your account still allows you to do so.', 'iva' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( esc_html__( 'Forum Name (Maximum Length: %d):', 'iva' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php esc_html_e('Forum Type:', 'iva' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php esc_html_e('Status:', 'iva' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php esc_html_e('Visibility:', 'iva' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php esc_html_e('Parent Forum:', 'iva' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => esc_html__( '(No Parent)', 'iva' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php esc_html_e('Submit', 'iva' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php printf( esc_html__( 'The forum %s is closed to new content.', 'iva' ), '&#8216;'.bbp_get_forum_title().'&#8217;' ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php is_user_logged_in() ? esc_html_e('You cannot create new forums.', 'iva' ) : esc_html_e('You must be logged in to create new forums.', 'iva' ); ?></p>
		</div>
	</div>
