...
class DTMFramework_Option_Gallery extends DTMFramework_Options {
  ...
  public function output(){
    ...
    $add    = ( ! empty( $this->field['add_title'] ) ) ? $this->field['add_title'] : esc_html__( 'Add Gallery', 'iva' );
    $edit   = ( ! empty( $this->field['edit_title'] ) ) ? $this->field['edit_title'] : esc_html__( 'Edit Gallery', 'iva' );
    $clear  = ( ! empty( $this->field['clear_title'] ) ) ? $this->field['clear_title'] : esc_html__( 'Clear', 'iva' );
    ...
    if( ! empty( $value ) ) {
      ...
      foreach ( $values as $id ) {
        ...
        echo '<li><img src="'. $attachment[0] .'" alt="'.esc_attr__( 'Image', 'iva' ).'" />'.$show_id_html.'</li>';
      }
